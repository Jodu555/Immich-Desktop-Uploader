<template>
	<div class="container">
			...
			<div v-for="(path, idx) in uploadPaths" :key="idx" class="path-item">
				<div class="path-header">
					...
					<div style="display: flex; gap: 10px">
						<button @click="triggerPath(idx)" class="btn-success">Trigger</button>
				...
				<div class="form-group">
					...
					<textarea v-model="path.cronExpressions" placeholder="0 0 * * * (daily at midnight)&#10;0 12 * * 0 (Sundays at noon)" rows="3"></textarea>
				...
				<div class="next-run" v-if="path.nextRun">Next upload: {{ new Date(path.nextRun).toLocaleString() }}</div>
			</div>
		...
		<section class="actions-section">
			...
			<button @click="startScheduler" :disabled="isSchedulerRunning" class="btn-success">Start Scheduler</button>
			<button @click="stopScheduler" :disabled="!isSchedulerRunning" class="btn-warning">Stop Scheduler</button>
		</section>
...
const triggerPath = (idx: number) => {
	const path = uploadPaths.value[idx];
	if (!path.directory) {
		addLog('Cannot trigger upload: Directory is empty', 'error');
		return;
	}
	invoke('trigger_upload', {
		directory: path.directory,
		recursive: path.recursive,
	})
		.then(() => {
			addLog(`Manually triggered upload for ${path.directory} Finished`, 'success');
		})
		.catch((e) => {
			console.log(e);
			addLog(`Failed to trigger upload for ${path.directory}: ${e}`, 'error');
		});
};
